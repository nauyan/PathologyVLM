...
# custom logger
loger = lg.getLogger(__name__)
loger.setLevel(lg.INFO)  # Set the logging level
...
class InternVLChatModel(PreTrainedModel):
    ...
    def __init__(
        ...
        if vision_model is not None:
            loger.info(f"setting the virchow model instead of defalut model")
        ...
        self.mlp1 = nn.Sequential(
            ...
            # nn.LayerNorm(5200),
            # nn.Linear(5200, llm_hidden_size),
            nn.GELU(),
        ...
        if config.use_llm_lora:
            logger.info("applying llm lora")
            self.wrap_llm_lora(
                r=config.use_llm_lora, lora_alpha=2 * config.use_llm_lora
            )

    # converting dimension from 1280 to 1024
    def compress_3d_tensor_mean_torch(self, tensor, target_dim=1024):
    ...
    def draw_heat_map(self, one_d_tensor, path="heat.png"):
        heatmap_data = one_d_tensor.reshape(1, -1).to("cpu").to(torch.float16)

        # Create the heatmap
        plt.imshow(heatmap_data, cmap="viridis", aspect="auto")
        plt.colorbar()  # Show color scale
        plt.title("Heat Map for 1D Tensor using Matplotlib")
        plt.xlabel("Index")
        plt.ylabel("Value")
        plt.xticks(ticks=np.arange(10), labels=np.arange(10))  # Label x-axis
        plt.yticks(ticks=[0], labels=["Value"])  # Label y-axis
    ...
    def extract_feature(self, pixel_values):
        ...
        # loger.info(f'importing virchow: ')
        # vision_model = get_virchow_model()
        # loger.info(f'default config of virchow is: {vision_model.default_cfg}')

        # vit_embeds = self.vision_model(pixel_values.to(torch.float32)).to(torch.bfloat16)
        vit_embeds = self.vision_model(pixel_values).to(torch.bfloat16)
        vit_embeds = vit_embeds[:, 5:]
        # self.draw_heat_map(vit_embeds[0][2],path='heat1.png')
        # vit_embeds = vit_embeds[:, 1:, :]
        # loger.info(f'vit_embeds after 1: shape = {vit_embeds.shape}')
        vit_embeds = self.compress_3d_tensor_mean_torch(vit_embeds)

        # self.draw_heat_map(vit_embeds[0][2],path='heat2.png')
    ...
    def chat(
        ...
        if num_patches_list is None:
            ...
            logger.info(f"num of patches are : {num_patches_list}")
        ...
        logger.info(f"num_path_list DIMS = {torch.tensor(num_patches_list).shape}")
